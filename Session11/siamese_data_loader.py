...
class siamese_data_loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        i1 = min((index-300)%len(self.image_list), (index+300)%len(self.image_list))
        i2 = max((index-300)%len(self.image_list), (index+300)%len(self.image_list))

        if i1==i2:
            i2+=1
        index2 = np.random.randint(i1, i2)   ### get a random pair. There are much better ways to do this.
        if index2==index:
            index2 -= np.random.randint(0,50)
        #print (index, index2)
        ...
        if image_1 is None:
            ...
            image_1 = np.zeros((self.resize_shape[1], self.resize_shape[0]))
            if self.resize:
        ...
        if image_2 is None:
            ...
            image_2 = np.zeros((self.resize_shape[1], self.resize_shape[0]))
            if self.resize:
